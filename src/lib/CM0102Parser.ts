  ...
  private parseStaff(): CMRawStaff[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      staffList.push({
        ...
        wage,
        value,
        adaptability:   Math.abs(adaptability),
        ambition:       Math.abs(ambition),
        determination:  Math.abs(determination),
        loyalty:        Math.abs(loyalty),
        pressure:       Math.abs(pressure),
        professionalism:Math.abs(professionalism),
        sportsmanship:  Math.abs(sportsmanship),
        temperament:    Math.abs(temperament),
        playerId,
      });
    }
